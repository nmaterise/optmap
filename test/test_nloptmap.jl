...
function solve_model(solver, model_log, solver_options=Dict())
    ...
    #regular function
    total_cost_f(amt_per_food) = sum([cost_per_food[f] * amt_per_food[f] for f
                                      in 1:n_foods])
    ...
    for r in 1:n_resources
        push!(constraints_f, (amt_per_food) ->
    ...
    return total_cost_f, constraints_f, variable_names, constraint_names,
    starting_point, solution_point, obj_val
end

#The datatype used in the tuples is used to determine if the variable is "real"
#or integer
